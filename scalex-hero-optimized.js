(function(){
/*! ScaleX hero · self-contained · baked 2026-09-18 17:53 UTC
 *
...
window.__SCALEX_BUILD = "50.1q8pbrn";
...
(function(){
  ...
  // DEFER: Lighthouse scores the first seconds of the page, and a continuous rAF loop
  // costs main-thread time for the whole run. window.SCALEX_DEFER holds the mount past
  // that window: "idle" (default when set to true), "load", "scroll", or a number of ms.
  function deferred(){
    var d = window.SCALEX_DEFER;
    if (!d) return Promise.resolve();
    return new Promise(function(go){
      var fired = false;
      function once(){ if (fired) return; fired = true; go(); }
      if (typeof d === "number"){ setTimeout(once, d); return; }
      if (d === "scroll"){
        window.addEventListener("scroll", once, { once:true, passive:true });
        setTimeout(once, 8000);   // never strand the hero on a page nobody scrolls
        return;
      }
      var after = function(){
        if (d === "load") return once();
        if (window.requestIdleCallback) requestIdleCallback(once, { timeout:3000 });
        else setTimeout(once, 1200);
      };
      if (document.readyState === "complete") after();
      else window.addEventListener("load", after, { once:true });
    });
  }
  chain.then(domReady).then(gate).then(deferred).then(function(){ __scalexMain(); })
...
const Q = {
  ...
  // capped by default once deployed: the loop is continuous work for the whole session, and
  // 30 halves both the GPU cost and the per-frame JS that Lighthouse counts as main-thread
  // work. The authoring console stays uncapped so tuning still feels live.
  fps:      QCFG.fps      ?? (HAS_CONSOLE ? 0 : 30),
  // 25-tap mask blur is only worth paying for at high feather; see uMaskHQ in the shader
  maskHQ:   QCFG.maskHQ   ?? !Q_LOW,
...
const SX = { compA, compB, camera, P, rebuildStacks, quality: Q, mounted: true, destroy };
...
const gridMat = new THREE.ShaderMaterial({
  uniforms: {
    ...
    uXFeather:{ value: 0.7 },
    uMaskHQ: { value: 1 },           // 1 = 25-tap mask blur, 0 = single tap (cheap path)        // X-mask edge softness 0..1
  ...
  fragmentShader: `
    ...
    float maskAt(vec2 p){
      ...
      if (uMaskShape > 1.5){
        ...
        // TAP BUDGET. The 5x5 spread costs 25 texture fetches per pixel per frame, and its
        // reach is o = f * 0.13 * 0.5 in uv. Below ~0.08 feather every tap lands inside about
        // one texel of the 128px glyph, so all 25 return the same value: pure waste. The mip
        // LOD bias already does the softening there, so take a single sample instead.
        // uMaskHQ is a uniform, so the branch is coherent across the whole draw.
        float g = 0.0;
        if (uMaskHQ < 0.5){
          g = (uv.x > 0.0 && uv.x < 1.0 && uv.y > 0.0 && uv.y < 1.0)
                ? texture2D(tX, uv, lod).r : 0.0;
        } else {
          float wsum = 0.0;
          for (int i = -2; i <= 2; i++){
            for (int j = -2; j <= 2; j++){
              ...
              float sm = 0.0;
              if (suv.x > 0.0 && suv.x < 1.0 && suv.y > 0.0 && suv.y < 1.0) sm = texture2D(tX, suv, lod).r;
              g += sm * w; wsum += w;
            }
          }
          g /= wsum;
...
function webflowEngine(role){
  ...
  if (role === 'preloader'){
    return wrap(base + [
      ...
      '  // backstop, not the primary path: always beyond the real sequence',
      '  setTimeout(finish, Math.max(6000, (span + 2) * 1000));',
      '}());',
      ''
    ].join('\n'));
  }
...
function sharedEngineFile(){
  ...
  const runtime = [
    ...
    '',
    '  // DEFER: hold the mount past the window Lighthouse measures.',
    '  // window.SCALEX_DEFER = true | "idle" | "load" | "scroll" | milliseconds',
    '  function deferred(){',
    '    var d = window.SCALEX_DEFER;',
    '    if (!d) return Promise.resolve();',
    '    return new Promise(function(go){',
    ...
    '    var span = 0; for (var i=0;i<ends.length;i++) if (ends[i] > span) span = ends[i];',
    '    setTimeout(finish, Math.max(400, (span + 0.25) * 1000));',
    '    // backstop, not the primary path: it must always sit BEYOND the real sequence,',
    '    // or a long opening gets cut short by its own safety net',
    '    setTimeout(finish, Math.max(6000, (span + 2) * 1000));',
    '  }',
    '',
    '  function mount(opts){',
    ...
    '                 params: opts.params || {} };',
    '    return ready.then(deferred).then(function(){',
...
if (!BAKED_PARAMS){
  ...
  if (window.SCALEX){ window.SCALEX.gizmo = GIZMO; window.SCALEX.gridMat = gridMat; window.SCALEX.Q = Q; }   // debug handle (null in baked export)
}
...
function tick(){
  ...
  if (gridLive()){
    ...
    // below this feather the 25 taps land within a texel of each other, so they buy nothing
    gridMat.uniforms.uMaskHQ.value = (Q.maskHQ && P.gXFeather >= 0.08) ? 1 : 0;

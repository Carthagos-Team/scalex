/* =========================================================================
   SCALEX — Hero WebGL Background (fractal fly-through) — VARIANTE ALT10
   Arquivo unico: parametros visuais (COOKED_PARAMS) + motor completo.

   Diferenca desta variante pra "fewerX" (base): calma, premium, discreta.
   Intro 10x mais lenta (10s vs 3.5s), reatividade ao mouse quase nula
   (mReact 0.1 vs 1.4), zoom de camera bem mais sutil (1->0.5 vs 2->1.76).

   Motor tambem mais recente: corrige um bug de flickering entre camadas
   transparentes (renderOrder deterministico) e ja vem com 3 presets de
   composicao aprovados (A/B/C) em vez de so 2 - mas so o preset ativo em
   COOKED_PARAMS (comp:"B" nesta variante) importa em producao.

   O motor desenha direto num <canvas class="hero_canvas"> ja existente
   no Designer (position:fixed, inset:0, z-index:-1) - nao cria seu
   proprio canvas nem precisa de bootstrap no Webflow.

   Sem checagem de pathname interna - o escopo por pagina e controlado
   por qual pagina do Webflow inclui este <script src> especificamente.

   COMO USAR NO WEBFLOW:
   1. Suba este arquivo num repositorio publico do GitHub.
   2. Na pagina especifica que vai usar esta variante (Page Settings >
      Custom Code, NAO no Site Settings sitewide), adicione:

      <script src="https://cdnjs.cloudflare.com/ajax/libs/three.js/r128/three.min.js"></script>
      <script src="https://cdn.jsdelivr.net/gh/SEU-USUARIO/SEU-REPO@main/scalex-hero-webgl-alt10.js"></script>

   3. Garanta que essa pagina tem um <canvas class="hero_canvas"> com
      position:fixed, inset:0, z-index:-1 no componente Hero.
   ========================================================================= */

window.COOKED_PARAMS={"px":2,"levels":7,"lens":600,"lensF":4,"lensOn":true,"lensStr":0.6,"lensFx":{"reveal":true,"brighten":true,"invert":false,"desat":true,"ripple":false,"condense":false},"par":1,"drift":1,"dither":true,"comp":"B","ditherMode":"bayer","dot":1.3,"glow":0,"glowR":0.6,"glowOn":true,"order":"ditherTop","ui":false,"introDur":10,"introStagger":0.03,"introEaseIn":2,"introEaseOut":6,"introOffX":-4,"introOffY":1,"introOffZ":0,"introSpin":5,"partFan":false,"echoOn":false,"mouseOn":true,"mMove":0.05,"mScale":0.05,"mRot":0,"mDelay":1.32,"mInvert":false,"mZPush":0.05,"mRoll":0.05,"mReact":0.1,"mReactR":100,"mCasAmt":0.05,"mCasDelay":1,"scrTurn":0,"scrDolly":0,"scrRise":10,"scrDelay":0.01,"scrCas":1,"bScroll":"rotate","bAxis":"z","bRot":0,"bScale":0.6,"grHue":-15,"grSat":0.7,"grBright":1.48,"tintOn":true,"tintA":"#2715b2","tintB":"#2c45aa","tintAmt":1,"tintDesat":1,"tintGain":2,"tintAngle":-15,"gradOn":true,"gradA":"#5451d6","gradB":"#0f145c","gradAngle":157,"gradGain":1.7,"blobOn":true,"blobSize":910,"blobOp":0.18,"blobFeather":60,"gCell":29,"gBase":0.26,"gBright":1.6,"gScale":0.5,"gMaskR":320,"gMaskShape":"x","gVig":1,"gVigR":0.3,"gIntroDur":1.8,"gEaseIn":2,"gEaseOut":4,"gIntroDir":"centerOut","gExpandOn":true,"gExpandFrom":0.35,"gExpandDur":1.6,"gXFrom":0.3,"gXSpin":0,"gRot":0,"gMRot":0,"gGap":0.24,"gTileSc":1.45,"gXSize":5,"gXFeather":0.36,"gMaskOp":0.6,"gPar":0.3,"gScaleM":0.05,"gSphere":0,"gDelay":0.3,"gMaskDelay":0.52,"gMaskEase":false,"gRevealDur":0.55,"gRevealDelay":4,"gRevealMode":"fade","gMouseMode":"parallax","gParAmt":50,"gXFade":1,"iters":7,"s0":1,"s1":0.5,"p0x":-0.754,"p0y":-0.628,"p0z":-5.97,"p1x":-7.05,"p1y":-2.9,"p1z":-5.3,"r0x":7,"r0y":26,"r0z":6,"r1x":0,"r1y":1,"r1z":30,"delayD":3,"delayS":0.13,"skew":0,"stx":1,"sty":0.8,"frontFade":0,"opMul":2.2,"tintHold":0.6};
...
/* =========================================================================
   ...
   ========================================================================= */
(() => {
/* Guard: so quando GSAP/ScrollTrigger ja estiverem prontos (carregados
   nativamente pelo Webflow, sem precisar de outro <script src> aqui). O
   escopo por pagina e controlado por qual pagina inclui este <script src>,
   nao por uma checagem de pathname aqui dentro. */

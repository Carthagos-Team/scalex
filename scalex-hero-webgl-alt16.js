...
if (!window.COOKED_PARAMS){
  ...
  const HALF_PI = Math.PI / 2, ARC_PAD = HALF_PI * 0.16, ARC_SWEEP = HALF_PI - 2 * ARC_PAD;
    ...
    const G = {
      ...
      pickAt(ndc){
        ...
        ENDS.forEach(which => {
          ...
          const bias = (which === this.target) ? 0 : 4;
          for (let i = 0; i < 3; i++){
            ...
            if (dM + bias < bestScore){ bestScore = dM + bias; best = { end:which, kind:'move', axis:AX[i] }; }
            ...
            for (let s = 0; s <= 24; s++){
              ...
              if (prev && prev[2] < 1 && pp[2] < 1){
                ...
                if (dR + bias < bestScore){ bestScore = dR + bias; best = { end:which, kind:'rotate', axis:AX[i] }; }
              }
              prev = pp;
            }
          }
        ...
        if (!best) return null;
        const raw = bestScore - ((best.end === this.target) ? 0 : 4);
        return raw <= 15 ? best : null;
      },

window.COOKED_PARAMS={"px":2,"levels":7,"lens":600,"lensF":4,"lensOn":true,"lensStr":0.6,"lensFx":{"reveal":true,"brighten":true,"invert":false,"desat":true,"ripple":false,"condense":false},"par":0.1,"drift":2,"dither":true,"comp":"B","ditherMode":"bayer","dot":1.3,"glow":0,"glowR":0.6,"glowOn":true,"order":"ditherTop","ui":false,"introDur":5,"introStagger":0.02,"introEaseIn":2,"introEaseOut":5,"introOffX":0,"introOffY":0,"introOffZ":0,"introSpin":10,"partFan":false,"echoOn":false,"introMode":"fan","introDelay":0,"introSpinX":-5,"introSpinY":-5,"introScale":1,"introOffX1":0,"introOffY1":0,"introOffZ1":0,"introSpinX1":0,"introSpinY1":0,"introSpin1":10,"introScale1":1,"mouseOn":true,"mMove":0.1,"mScale":0.1,"mRot":0.1,"mDelay":1.32,"mInvert":false,"mZPush":0,"mRoll":0,"mReact":0.6,"mReactR":220,"mCasAmt":0.3,"mCasDelay":0.92,"scrTurn":0,"scrDolly":0,"scrRise":10,"scrDelay":0.01,"scrCas":1,"bScroll":"rotate","bAxis":"z","bRot":0,"bScale":0.6,"grHue":-15,"grSat":0.7,"grBright":1.48,"tintOn":true,"tintA":"#2715b2","tintB":"#2c45aa","tintAmt":1,"tintDesat":1,"tintGain":2,"tintAngle":-15,"gradOn":true,"gradA":"#5451d6","gradB":"#0f145c","gradAngle":157,"gradGain":1.7,"blobOn":true,"blobSize":910,"blobOp":0.18,"blobFeather":60,"gCell":29,"gBase":0.26,"gBright":1.6,"gScale":0.5,"gMaskR":320,"gMaskShape":"x","gVig":1,"gVigR":0.3,"gIntroDur":3,"gEaseIn":2,"gEaseOut":2,"gIntroDir":"centerOut","gExpandOn":true,"gExpandFrom":0.65,"gExpandDur":1.6,"gXFrom":0.3,"gXSpin":0,"gRot":0,"gMRot":0,"gGap":0.24,"gTileSc":1.45,"gXSize":5,"gXFeather":0.36,"gMaskOp":0.6,"gPar":0.05,"gScaleM":0,"gSphere":0,"gDelay":0.3,"gMaskDelay":2,"gMaskEase":false,"gRevealDur":1,"gRevealDelay":1,"gRevealMode":"fade","gMouseMode":"parallax","gParAmt":50,"gXFade":1,"iters":6,"s0":2,"s1":0.1,"p0x":-3.653,"p0y":-0.535,"p0z":-2.756,"p1x":-7.118,"p1y":-3.924,"p1z":9.439,"r0x":0,"r0y":1,"r0z":33,"r1x":0,"r1y":32,"r1z":40,"delayD":3,"delayS":0.13,"skew":0,"stx":1,"sty":0.8,"frontFade":0,"opMul":2.2,"tintHold":0.6};
/* =========================================================================
...
/* PRISTINE is only needed by the tuning console's BAKE button. A baked build
   never calls it, so skip the whole-document serialisation and the retained
   string: pure startup cost for something that can never run here. */
const PRISTINE = window.COOKED_PARAMS ? '' :
...
/* ---------- quality tier (mobile performance) ------------------------------
   This renderer is fill-rate bound: every frame draws a stack of large
   transparent quads (heavy overdraw, no early-z rejection) and then runs
   fullscreen post passes. Mobile GPUs are tile-based, so pixel count and
   render-target churn cost far more there than on desktop.

   WEBFLOW: set this BEFORE the hero script if you want to override anything.
     <script>window.SCALEX_QUALITY = 'low';<\/script>          force a tier
     <script>window.SCALEX_QUALITY = { maxIters:6 };<\/script>  keep all copies
     <script>window.SCALEX_QUALITY = { adaptive:false };<\/script>  fixed res
   Leave it unset for automatic detection. Read the resolved tier at runtime with
   window.SCALEX.quality, and the live render scale with window.SCALEX.dpr
--------------------------------------------------------------------------- */
...
const Q = {
  mobile:    Q_LOW,
  dpr:       QCFG.dpr       ?? (Q_LOW ? 1.0 : 1.5),
  maxPixels: QCFG.maxPixels ?? (Q_LOW ? 1600000 : 4200000), // absolute ceiling per pass
  glow:      QCFG.glow      ?? !Q_LOW,      // the 13-pass bloom pyramid
  maxIters:  QCFG.maxIters  ?? (Q_LOW ? 4 : 999),
  hover:     QCFG.hover     ?? !Q_COARSE,   // no hover exists on touch
  lens:      QCFG.lens      ?? true,        // set false to drop the per-pixel lens math
  fps:       QCFG.fps       ?? 0,           // 0 = uncapped; 30 halves GPU load
  pauseOffscreen: QCFG.pauseOffscreen ?? true,
  adaptive:  QCFG.adaptive  ?? true,        // step resolution down if frames are slow
  adaptMs:   QCFG.adaptMs   ?? 30,          // median frame time that counts as struggling
  adaptSteps:QCFG.adaptSteps ?? 3           // how many 20% reductions are allowed
};

/* pause entirely when the hero is scrolled past or the tab is hidden. the canvas
   is position:fixed, so once .hero is gone the next section covers it. */
...
/* ---------- adaptive resolution -------------------------------------------
   A DPR cap is a guess made before we know anything about the device. This
   watches real frame times and, if the median is genuinely bad, reduces the
   render scale and re-sizes. Only ever downward, with a floor and a cooldown so
   it cannot oscillate. */
let ADAPT = 1, _adTimes = [], _adSteps = 0, _adLast = 0, _adCool = 0;
function adaptTick(now){
  if (!Q.adaptive || _adSteps >= Q.adaptSteps) return;
  if (_adLast){
    _adTimes.push(now - _adLast);
    if (_adTimes.length > 30) _adTimes.shift();
  }
  _adLast = now;
  if (_adTimes.length < 30 || now < _adCool) return;
  const sorted = _adTimes.slice().sort((a, b) => a - b);
  if (sorted[15] > Q.adaptMs){
    _adSteps++;
    ADAPT *= 0.8;
    _adTimes.length = 0;
    _adCool = now + 1500;
    if (typeof requestResize === 'function') requestResize(true);
...
const renderer = new THREE.WebGLRenderer(__sxOwnCanvas ? { canvas: stage, antialias:false, stencil:false,
  powerPreference: Q.mobile ? 'default' : 'high-performance' } : { antialias:false, stencil:false,
  powerPreference: Q.mobile ? 'default' : 'high-performance' });
/* mutable: the pixel budget and the adaptive step both fold into it */
let DPR = 1;
function computeDPR(w, h){
  let d = Math.min(window.devicePixelRatio || 1, Q.dpr) * ADAPT;
  if (Q.maxPixels > 0){
    const px = w * h * d * d;
...
/* pixel ratio is set in resize(), which runs before the first frame and is
   the only place that knows the viewport, the budget and the adaptive step. */
...
const P = Object.assign({
  ...
  gVig:1.0, gVigR:0.3, gIntroDur:3, gEaseIn:2, gEaseOut:2, gIntroDir:'centerOut', gExpandOn:true, gExpandFrom:0.65, gExpandDur:1.6, gXFrom:0.3, gXSpin:0,
  gRot:0, gMRot:0, gGap:0.24, gTileSc:1.45, gXSize:5, gXFeather:0.36, gMaskOp:0.6,
  gPar:0.05, gScaleM:0, gSphere:0, gDelay:0.3, gMaskDelay:2, gMaskEase:false, gRevealDur:1, gRevealDelay:1, gRevealMode:'fade', gMouseMode:'parallax', gParAmt:50, gXFade:1,
...
window.SCALEX = { compA, compB, camera, P, rebuildStacks, quality: Q,
                  get dpr(){ return DPR; }, get adaptSteps(){ return _adSteps; } };   // debug handles (cookString attached later)

/* ---------- post pipeline: dither pass + multi-octave glow (Deep-Glow-style) ---------- */
const rt    = new THREE.WebGLRenderTarget(2, 2);   // scene render
/* blit-only targets: they are never rendered into with geometry, so a depth
   and stencil buffer on each was pure allocated bandwidth */
const NO_DEPTH = { depthBuffer:false, stencilBuffer:false };
const auxRT = new THREE.WebGLRenderTarget(2, 2, NO_DEPTH);   // intermediate full-res
const outRT = new THREE.WebGLRenderTarget(2, 2, NO_DEPTH);   // composited frame -> grade pass
...
for (let i = 0; i < GLOW_LEVELS; i++){
  downRT.push(new THREE.WebGLRenderTarget(2, 2, NO_DEPTH));
  upRT.push(new THREE.WebGLRenderTarget(2, 2, NO_DEPTH));
}
...
function resize(){
  ...
  DPR = computeDPR(w, h);        // pixel budget + adaptive step fold in here
  renderer.setPixelRatio(DPR);  // canvas backing store follows the same scale
  ...
  for (let i = 0; i < GLOW_LEVELS; i++){
    ...
    // only (re)allocate the glow mips when glow can run. this look bakes
    // glow:0, so these were 12 pointless GPU allocations on every resize.
...
// Mobile browsers fire `resize` continuously while the URL bar hides and shows
// during scroll, and only the height changes. Reallocating render targets on every
// one of those is the main cause of scroll stutter on phones, so height-only jitter
// inside the browser-chrome range is ignored and real resizes are coalesced.
...
ScrollTrigger.create({
  trigger: '.hero', start: 'bottom bottom', end: 'bottom 15%', scrub: 0.6,
...
function refreshPanel(){
  ...
  setChk('uDither', P.dither); setChk('uGlowOn', P.glowOn); setChk('uLensOn', (P.lensOn && Q.lens));
...
if ($('uLensOn')){
  $('uLensOn').checked = (P.lensOn && Q.lens);
...
function refreshEntrySliders(){
  ...
  for (const f in IE_SLIDERS){
    ...
    el.value = P[map[f]];
    const o = el.parentElement.querySelector('output'); if (o) o.textContent = P[map[f]];
  }
}
for (const f in IE_SLIDERS){
  const el = $(IE_SLIDERS[f]); if (!el) continue;
  el.addEventListener('input', e => {
    ...
    P[IE_KEYS[introEdit][f]] = v;
    const o = e.target.parentElement.querySelector('output'); if (o) o.textContent = v;
  });
}
...
function tick(){
  ...
  // hero offscreen or tab hidden: draw nothing. delta is still drained so time
  // does not jump on resume.
  if (!RENDER_ON){ clock.getDelta(); return; }
  const _now = performance.now();
  if (Q.fps > 0){
    if (_now - _fpsLast < (1000 / Q.fps) - 1) return;
    _fpsLast = _now;
  }
  adaptTick(_now);
  ...
  ditherMat.uniforms.uLensOn.value = (P.lensOn && Q.lens) ? 1 : 0;
